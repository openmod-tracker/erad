...
class FireModel(BaseDisasterModel):
    ...
    def from_wildfire_name(cls, wildfire_name: str) -> "FireModel":
        ...
        fire_data = pd.read_sql(f"SELECT * FROM {HISTROIC_FIRE_TABLE} WHERE firename = '{wildfire_name}';", conn)
        if fire_data.empty:
            raise ValueError(f"Fire '{wildfire_name}'  not found in  table '{HISTROIC_FIRE_TABLE}' in the database")
        conn.close() 
        ...
        return cls(
            name = wildfire_name,
            timestamp=fire_data["discoverydatetime"].values[0].astype('datetime64[ms]').astype(datetime),
