...
class AssetState(Component):
    ...
    def calculate_earthquake_vectors(self, asset_coordinate: Point, hazard_model: hz.EarthQuakeModel):
        """
        Sources:
        ...
        mmi = 3.31 + 1.28 * hazard_model.magnitude - 1.42 * math.log10(hypocentral_distance.to("km").magnitude) # Modified Mercalli Intensity 
    ...
    def survival_probability(self) -> float:
        ...
        water_flow_survival_probability = self.flood_velocity.survival_probability  if self.flood_velocity else 1
        water_level_survivial_probability = self.flood_depth.survival_probability  if self.flood_depth else 1
        fire_survival_probability = self.fire_boundary_dist.survival_probability  if self.fire_boundary_dist else 1
        pgv_survival_probability = self.peak_ground_velocity.survival_probability  if self.peak_ground_velocity else 1
        pga_survival_probability = self.peak_ground_acceleration.survival_probability  if self.peak_ground_acceleration else 1
        
        return wind_survival_probability * \
...
class Asset(Component):
    ...
    def update_survival_probability(self, time_stamp: datetime, hazard_model: hz.BaseDisasterModel, frag_curves):
    ...
    def calculate_probabilities(self, asset_state:AssetState, frag_curves):
        fields = [l for l in list(asset_state.model_fields.keys()) if l not in ['uuid', 'name', 'timestamp']]
        for field in fields:
